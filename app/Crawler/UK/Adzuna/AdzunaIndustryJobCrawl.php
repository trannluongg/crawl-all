...
class AdzunaIndustryJobCrawl extends CrawlerBase
{
    private $params;
    ...
    private function __configSelector(): array
    {
        return [
            'jobs'        => [
                'selector' => 'css: .ui-search-results > div.a',
                'link'     => false,
                'multiple' => true,
                'child'    => [
                    'heading' => [
                        'selector' => 'css: .description-column a',
                        'link'     => false,
                        'multiple' => false
                    ]
                ],
            ...
            'also_search' => [
                'selector' => 'css: .related-searches a',
                'link'     => false,
                'multiple' => true
            ],
            'link_next'   => [
                'selector' => 'xpath: //a[contains(text(), "next ❯")]',
                'link'     => false,
                'multiple' => false,
            ]
        ];
            ...
            foreach ($children as $child)
            {
                if ($child instanceof Node)
                {
                    if ($child->text() != "")
                    {
                        ...
                        $link         = $elementTitle->attribute('href');
                        if ($title != '' && $link != '')
                        {
                            $dataInsert = [
    ...
    private function __getAlsoSearch(string $site = '', array $options = [])
    {
        ...
        if (count($childrenAlsoSearch) > 0 && $meta == null)
        {
            ...
            foreach ($childrenAlsoSearch as $alsoSearch)
            {
                ...
                $link  = $alsoSearch->attribute('href');
